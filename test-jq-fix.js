...
try {
  ...
  const dockerOutput = execSync('docker compose -f docker-compose.test.yml ps --format json', { encoding: 'utf8' });
  console.log('Docker compose output:');
  console.log(dockerOutput);
  
  // Test the fixed jq command
  const jqCommand = 'docker compose -f docker-compose.test.yml ps --format json | jq -s -r ".[] | select(.Service == \\"postgres-test\\") | .Health"';
  ...
  try {
    const nonExistentResult = execSync('docker compose -f docker-compose.test.yml ps --format json | jq -s -r ".[] | select(.Service == \\"non-existent\\") | .Health"', { encoding: 'utf8', shell: '/bin/bash' }).trim();
