    ...
    it('should detect eslint-plugin-react-hooks version mismatch', () => {
      ...
      if (packageJsonVersion && lockfileEntry) {
        ...
        const lockfileSpecifier =
          typeof lockfileEntry === 'string' ? lockfileEntry : lockfileEntry.specifier;
        expect(
          lockfileSpecifier,
            ...
            try {
              ...
              Object.keys(packageJsonSpecs).forEach(dep => {
                if (lockfileSpecs[dep] && lockfileSpecs[dep] !== packageJsonSpecs[dep]) {
                  mismatches.push(
                    `${dep}: lockfile=${lockfileSpecs[dep]}, package.json=${packageJsonSpecs[dep]}`,
                  );
                }
              });
  ...
  describe('Prevention Tests', () => {
    ...
    it('should have valid semver ranges for all dependencies', () => {
      ...
      Object.entries({
        ...
        expect(version as string, `${dep} should have valid semver: ${version}`).toMatch(
          semverRegex,
        );
      });
