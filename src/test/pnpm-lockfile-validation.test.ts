...
describe('PNPM Lockfile Validation - BDD Fix for dry-run Error', () => {
  ...
  beforeAll(() => {
    ...
    expect(existsSync(join(projectRoot, 'pnpm-lock.yaml')), 'pnpm-lock.yaml should exist').toBe(
      true,
    );
  ...
  describe('Lockfile Synchronization Tests', () => {
    it('should pass pnpm install --frozen-lockfile without errors', () => {
      try {
        ...
        if (errorMessage.includes('dry-run')) {
          throw new Error(
            `Invalid 'dry-run' option detected. This option doesn't exist in pnpm: ${errorMessage}`,
          );
        }
    ...
    it('should have consistent workspace dependencies', () => {
      ...
      expect(rootPackageJson.workspaces).toEqual(expect.arrayContaining(['backend', 'frontend']));
