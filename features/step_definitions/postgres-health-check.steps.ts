...
Then(
  'the health check command should use {string} environment variable',
  function (envVar: string) {
    const postgresService = dockerComposeConfig.services['postgres-test'];
...
Then('the health check should not contain hardcoded database names', function () {
  ...
  for (const name of hardcodedNames) {
    expect(testCommand).to.not.include(
      name,
      `Health check should not contain hardcoded database name: ${name}`,
    );
  }
});
...
Then(
  'the start_period should allow for container initialization \(at least {int}s\)',
