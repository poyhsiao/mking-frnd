# 使用 BDD 方法修复 PNPM 锁文件 CI 问题

## 🎯 概述

本文档详细说明了使用行为驱动开发 (BDD) 方法解决 GitHub Actions CI 流水线中
`ERR_PNPM_OUTDATED_LOCKFILE` 错误的综合解决方案。
...
1. **版本不匹配**：CI 使用 `PNPM_VERSION: '8'`，而 `backend/package.json` 指定
   `pnpm@8.15.1`
...
遵循 BDD 原则，在实施修复之前创建了全面的场景套件：

#### 场景套件 1：锁文件同步 (`tests/ci/pnpm-lockfile-sync.test.ts`)
...
#### 场景套件 2：冻结锁文件兼容性 (`tests/ci/pnpm-frozen-lockfile.test.ts`)
...
- [BDD 指南](./development/bdd-guidelines-zh.md)
- [CI/CD 配置](../.github/workflows/ci.yml)
- [测试策略](./testing/testing-strategy-zh.md)

---

**状态**：✅ 已完成 **日期**：2024-12-19 **方法**：行为驱动开发 (BDD)
**影响**：高 - 关键 CI 流水线稳定性
