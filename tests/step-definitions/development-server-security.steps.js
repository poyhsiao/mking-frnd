...
When("I make a request from an unauthorized origin", function () {
	...
	// Simulate CORS check
	const isAllowed =
		this.corsSettings.allowedOrigins.includes(unauthorizedOrigin);
...
Then(
	"the request should be rejected with appropriate CORS headers",
	function () {
...
Then("security headers should be properly configured", function () {
	...
	expect(this.securitySettings.headers).to.have.property(
