...
(0, cucumber_1.Given)('the PostgreSQL container is configured with proper environment variables', function () {
    const postgresService = this.dockerConfig.services['postgres-test'];
...
(0, cucumber_1.Given)('the PostgreSQL container is configured with environment variables', function () {
    const config = getDockerComposeConfig();
...
(0, cucumber_1.Given)('the PostgreSQL container has health check timeout of {int} seconds', function (timeout) {
    const config = getDockerComposeConfig();
    (0, chai_1.expect)(config.services['postgres-test'].healthcheck.timeout).to.equal(`${timeout}s`);
});

(0, cucumber_1.Given)('the health check has retry configuration of {int} attempts', function (retries) {
    const config = getDockerComposeConfig();
...
(0, cucumber_1.Then)('the container status should be {string}', function (expectedStatus) {
    const command = 'cd .. && docker-compose -f docker-compose.test.yml ps --format table postgres-test';
    const result = executeCommand(command);
...
(0, cucumber_1.Then)('the health check should pass', function () {
    const command = 'cd .. && docker-compose -f docker-compose.test.yml exec -T postgres-test pg_isready -U ${POSTGRES_TEST_USER} -d ${POSTGRES_TEST_DB}';
    const result = executeCommand(command);
    (0, chai_1.expect)(result.success).to.be.true;
});
...
(0, cucumber_1.Then)('the health check should retry on failure up to {int} times', function (retries) {
    const config = getDockerComposeConfig();
    (0, chai_1.expect)(config.services['postgres-test'].healthcheck.retries).to.equal(retries);
});
