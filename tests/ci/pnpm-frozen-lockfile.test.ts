...
describe('PNPM Frozen Lockfile CI Compatibility', () => {
  ...
  it('should have all backend dependencies properly locked', () => {
    ...
    Object.keys(expectedDeps).forEach(dep => {
      expect(expectedDeps[dep], `Dependency ${dep} should have a valid version specifier`).toMatch(
        /^[\^~]?\d+\.\d+\.\d+/,
      );
    });
  });
    ...
    if (versionMatch) {
      ...
      if (backendPackageJson.packageManager) {
        ...
        if (packageManagerMatch) {
          ...
          expect(ciPnpmVersion, 'CI pnpm version should match packageManager version').toBe(
  ...
  it('should have lockfile validation step in CI', () => {
    ...
    expect(ciConfig, 'CI should have lockfile validation step').toMatch(
      /name:\s*Validate lockfile/,
    );

    // Check that it provides helpful error messages
    expect(ciConfig, 'CI should provide helpful lockfile error messages').toMatch(
